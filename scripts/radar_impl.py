...
TOPICS = {
    "sst": ["sst", "solid-state transformer", "solid state transformer"], "sic": ["sic", "silicon carbide", "碳化硅"], "gan": ["gan", "gallium nitride", "氮化镓"],
    "gfm": ["grid-forming", "grid forming", "gfm", "构网型"], "pcs": ["pcs", "power conversion system", "变流器", "储能变流器"],
    "ai_dc": ["ai data center", "ai datacenter", "data center", "datacenter", "数据中心"], "dab": ["dab", "dual active bridge"], "cllc": ["cllc"], "llc": ["llc"],
    "800v": ["800v", "800 v", "800-volt", "800伏"], "magnetics": ["magnetics", "magnetic integration", "high-frequency magnetics", "磁性元件"]
}
...
def add_candidate(candidates, seen, item, source_name, source_type):
    ...
    if not any(k.lower() in text for k in KEYWORDS): return
...
def collect_events():
    ...
    for source_name, url, source_type in RSS_SOURCES:
        try: feed = feedparser.parse(url)
            ...
            print("RSS error:", source_name, repr(exc)); continue
    ...
    # Resolve and fetch source text before clustering. Events without usable original content are discarded.
    grounded = []
    for c in candidates:
    ...
    for article in sorted(candidates, key=lambda x: (x["score"], x["published_at"]), reverse=True):
        ...
        if cluster:
            ...
            if len(article["source_text"]) > len(cluster["primary"]["source_text"]):
                cluster["primary"].update({"summary": article["summary"], "source_text": article["source_text"]})
...
输出合法JSON，不要Markdown。每个事件生成两个真正不同的摘要：plain_summary 3-5句，面向普通读者；technical_summary 5-8句，面向电力电子工程师。
technical_summary只能讨论SOURCE_TEXT明确出现的器件、拓扑、控制、参数、实验、产品或产业化信息。SOURCE_TEXT没有明确写出的内容必须省略，不得推断。
论文必须以摘要/正文明确内容为依据；不能根据论文标题猜研究结论。企业宣传中的“首个/领先/量产”等只能作为来源声称，不能改写成客观事实。
...
def normalize_ai_event(ai_event, base):
    ...
    e["category"] = e.get("category") or base.get("category", []); e["sources"] = e.get("sources") or base.get("sources", [])
    e["plain_summary"] = clean(e.get("plain_summary", "")); e["technical_summary"] = clean(e.get("technical_summary", ""))
    ...
    ind = dict(e.get("industrialization") or {}); ind["stage"] = ind.get("stage") or "未知"; ind["status"] = ind.get("status", ""); ind["target"] = ind.get("target", ""); e["industrialization"] = ind
...
def main():
    tz = timezone(timedelta(hours=8)); now = datetime.now(timezone.utc).astimezone(tz); report_date = now.strftime("%Y-%m-%d")
    raw_events = collect_events()
    if not raw_events:
        ai = {"events": [], "directions": {}, "observations": [], "quality_notes": "本次没有获得具有可核验原文内容的强相关事件。"}; events = []
    else:
        try:
            ai = ask_deepseek(raw_events, report_date); by_id = {x["id"]: x for x in raw_events}
